...
def fill_rice_data(df):
    ...
    filled_rice_regular = rice_regular.alias("regular").join(
        rice_paddy.alias("paddy"),
        join_keys,
        "left"
    )
    ...
    #         )
    #         print(f"Replacing {col_name} in rice paddy with rice regular data")
    #     else:
    #         select_cols.append(F.col(f"paddy.{col_name}"))

    for col_name in rice_regular.columns:
        if col_name in trade_columns_to_replace:
            select_cols.append(
                F.col(f"paddy.{col_name}").alias(col_name)
            )
            print(f"Replacing {col_name} in rice regular with rice paddy data")
        else:
            select_cols.append(F.col(f"regular.{col_name}"))

    filled_rice_regular = filled_rice_regular.select(*select_cols)

    result_df = filled_rice_regular.union(other_crops)
